...
class AnalyticsServer {
                    ...
                    case "get_kpis":
                        return {
                            content: [
                                {
                                    type: "text",
                                    text: JSON.stringify(await this.getKPIs(args?.department), null, 2),
                                },
                            ],
                        };
                    case "get_trend":
                        return {
                            content: [
                                {
                                    type: "text",
                                    text: JSON.stringify(await this.getTrend(args?.days || 7, args?.department), null, 2),
                                },
                            ],
                        };
                    case "list_calls":
                        return {
                            content: [
                                {
                                    type: "text",
                                    text: JSON.stringify(await this.getRecentCalls(args?.limit || 20, args?.department), null, 2),
                                },
                            ],
                        };
                    default:
    ...
    async getKPIs(department) {
      ...
      `;
            ...
            const volumeResult = this.db.prepare(volumeQuery).get(...volumeParams);
        ...
        `;
            ...
            const selectionResult = this.db.prepare(selectionQuery).get(...selectionParams);
        ...
        `;
            ...
            const transferResult = this.db.prepare(transferQuery).get(...transferParams);
    ...
    async getTrend(days, department) {
    ...
    async getRecentCalls(limit, department) {
        try {
            let query;
            let params;
            if (department) {
